...
export default function GlyphPortal({
  ...
  const characters = Array.from(text).reduce<{ char: string; index: number }[]>((acc, char) => {
    ...
    const index = prev ? prev.index + prev.char.length : 0;
    acc.push({ char, index });
    return acc;
  }, []);
  ...
  return (
    <section ref={sectionRef} id={uid} className={className} aria-label={text}
      ...
      <style>{`
        ${q}{--gp-paper:#FFFFFF;--gp-ink:#111111;--gp-field:#FFFFFF;--gp-foreground:#111111;position:relative;isolation:isolate;background:var(--gp-paper);color:var(--gp-ink);font-family:var(--font-montserrat), "Montserrat", sans-serif;}
        ...
        ${q} [data-gp-marks]{fill:none;stroke:var(--gp-ink);opacity:.2;}
        ...
        ${q} [data-gp-letter]:focus-visible{outline:2px solid #1D4ED8;outline-offset:5px;}
        ...
        ${q} [data-gp-select]{min-height:44px;min-width:90px;border:1px solid rgba(17,17,17,0.15);border-radius:4px;background:var(--gp-paper);color:var(--gp-ink);padding:0 10px;font:inherit;}
        ${q} [data-gp-select]:focus-visible{outline:2px solid #1D4ED8;outline-offset:4px;}
        ...
        ${q} [data-gp-hint]{max-width:30ch;color:rgba(17,17,17,0.7);}
        ...
        ${q} [data-gp-default-copy]{color:rgba(17,17,17,0.7);font:16px/1.6 Arial,sans-serif;max-width:36ch;margin:0;}
      ...
      <div data-gp-pin>
        <div data-gp-field aria-hidden="true" inert>
          {background ?? <div data-gp-default-field style={{ position: "absolute", inset: 0, transform: "scale(var(--gp-field-scale,1))", background: "#FFFFFF" }} />}
        </div>
